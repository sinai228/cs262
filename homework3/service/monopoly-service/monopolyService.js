...
router.get("/player_game", readPlayerGames);
router.get("/player_game/:id", readPlayersGames);
router.get("/playergames", readjoinedGames);
...
function readjoinedGames(req, res, next) {
    db.many(`SELECT * FROM PLAYER, PlayerGame WHERE Player.ID = playerID`)
        .then(data => {
            returnDataOr404(res, data);
        })
        .catch(err => {
